...
3. When the model calls an OpenEMR tool, `openemrFetch` (`lib/openemr/api.ts`) reads the bearer token from the session and queries that user's OpenEMR API base.
...
5. Chat history, users, and votes persist to Postgres via Drizzle.
