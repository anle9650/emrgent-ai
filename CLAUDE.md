...
pnpm eval:scribe   # Live-model agent evals for the scribe flow (tests/evals/scribe/)
...
- `tests/evals/scribe/` — **live-model agent evals** for the scribe flow (`pnpm eval:scribe`, costs real gateway tokens). Sends dialogue-transcript kickoff messages to a live Kimi K2.5 agent running the production system prompt and the real OpenEMR tools against fixtures, then (a) deterministically checks the tool-call protocol (`checks.ts`) and (b) grades SOAP quality + documentation fidelity with LLM graders (`grader.ts`). The env split that makes this possible: `useMockModels` vs `useOpenEmrFixtures` in `lib/constants.ts` — `OPENEMR_FIXTURES=true` serves fixture data (with a stateful created-encounter overlay + `resetOpenEmrFixtureState()`) while models stay live; `PLAYWRIGHT=True` implies both. The script needs `--conditions=react-server` (neutralizes `server-only` in `lib/openemr/api.ts`) — always run it via the package script. Flags: `--case <id>`, `--trials <n>`, `--skip-graders`, `--json <path>`. Expect some live-model variance; failures print the tool-call trace and grader rationales.
