...
- **Capturing UI screenshots for visual checks**: boot the mock stack with `PLAYWRIGHT=True pnpm dev` (mock models + fixtures — no gateway calls, no OpenEMR session needed), then drive it with a standalone Playwright script run via `node`. A script outside the repo can't `import` `@playwright/test` (ESM resolves from the script's path, not cwd) — use `createRequire("<repo>/package.json")("@playwright/test")`. Trigger a mock flow (e.g. fill `multimodal-input` with an `/appointment/i` phrase and click `send-button`), then screenshot `[data-role='assistant']` or a specific element inside it; capture both themes by creating one browser context per `colorScheme: "light" | "dark"`. Shut the server down with SIGTERM afterwards (see below).
- **Driving the sidebar in Playwright scripts**: the sidebar starts **collapsed** in a fresh browser context (no `sidebar_state` cookie → `defaultOpen: false` in `app/(chat)/layout.tsx`), which hides chat-history links and puts a hover-only expand overlay over the logo that intercepts real-mouse clicks. Either seed the cookie before `page.goto` (`context.addCookies([{ name: "sidebar_state", value: "true", url: "http://localhost:3000" }])`) or click `sidebar-expand` first. Test ids: `sidebar-toggle` (collapse, expanded mode), `sidebar-expand` (expand, collapsed mode — opacity-0 but clickable), `sidebar-home-link` (logo → `/`), `sidebar-new-chat` (footer button → `/`), `sidebar-history-item` (chat links). Navigate between chats **client-side** via these (not `page.goto`, which reloads and clears SWR/useChat state — invisible to cache-related repros).
- **Stop dev servers gracefully — never `kill -9` (SIGKILL) one mid-compile.** Next traps SIGTERM and finalizes its Turbopack cache on the way out; SIGKILL can't be caught, so a compile in flight leaves `.next` half-written and the next start wedges on `○ Compiling / ...` — the app becomes unusable and every e2e `page.goto` times out with `net::ERR_ABORTED; maybe frame was detached?`. Use Ctrl-C or `kill`/`kill -15`; reserve `kill -9` for a process already ignoring SIGTERM. Recovery when it does happen: `rm -rf .next node_modules/.cache` and restart. Let Playwright manage its own webServer rather than force-killing servers/port-3000 holders around a run.
