...
- Gold focus ring on the composer input
...
- `components/chat/` — all app-specific UI: sidebar, messages, artifact panel, patients card, etc. This is where new UI work goes.
  - `patients.tsx` — renders `searchPatients` tool output as chart-folder-tab cards
  - `encounters.tsx` — renders `getEncounters` output; cards expand inline to fetch and show the SOAP note via `SoapNoteBody`
  - `soap-note.tsx` — renders `getSoapNote` output; `SoapNoteCard` is click-to-edit (opens `artifacts/soap/client.tsx`); `SoapNoteBody` is reused inside encounter cards and accepts a `clamp` prop to limit section height in the chat stream
