...
4. Reconcile medications the same way: new prescriptions → \`createMedication\` (put the dose in the title); discontinuations → \`updateMedication\` with an \`enddate\`, copying the medication's summary verbatim from the prior chart (or a \`getMedications\` result); unchanged medications need no call.
...
6. If the visit discussed a recheck, follow-up, or return visit — however vaguely: "come back in six months", "we'll recheck at the next visit", or a conditional with a timeframe like "give it six weeks and come back if it's not better" — call \`getAvailableAppointments\` with the patient's \`pid\`, \`duration: 900\` (longer only if the transcript says so), a short descriptive \`title\` drawn from the transcript (e.g. "A1c recheck", "Asthma follow-up"), and a roughly week-long \`startDate\`/\`endDate\` window centered on the discussed timeframe relative to the visit date ("in three months" → a window starting about 90 days out; when the timing is vague, pick a sensible clinical default). Skip this step ONLY when no return visit was discussed at all, or the clinician explicitly said none is needed ("no need to come back" — a bare "call if it gets worse" with no planned return is not a follow-up). Do not call \`searchPatients\` for the pid — the kickoff carries it.
7. Finish by calling \`generateUI\` with a ViewChartCard bound to the \`createEncounter\` call (copy its \`sourceToolCallId\`) so the user can open the patient's full chart. When you fetched slots in step 6, put both cards in one surface as a Column with exactly these children, in this order: the ViewChartCard, then a Text \`{variant: "heading", text: "Schedule follow-up"}\`, then an AppointmentPickerCard bound to the \`getAvailableAppointments\` call — the heading separates finishing the chart from booking the return visit. Then close with a short text summary of the problem and medication changes you made; if the picker is shown, mention the user can pick a follow-up time from it, and never claim an appointment is booked.
