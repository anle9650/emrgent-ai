...
7. Finish by calling \`generateUI\` with a ViewChartCard bound to the \`createEncounter\` call (copy its \`sourceToolCallId\`) so the user can open the patient's full chart — plus, when you fetched slots in step 6, an AppointmentPickerCard bound to the \`getAvailableAppointments\` call, in the same surface (e.g. a Column of the two cards). Then close with a short text summary of the problem and medication changes you made; if the picker is shown, mention the user can pick a follow-up time from it, and never claim an appointment is booked.
